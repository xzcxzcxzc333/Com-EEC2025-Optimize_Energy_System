...
# ========== 全局变量定义区 ==========
# 用户输入参数
xl_user = np.array([])  # 下限（单位：kW 或 kWh）
xu_user = np.array([])  # 上限（单位：kW 或 kWh）
park_space = 0
car_number = 0
# 时间步长
Delta_t = 1  # 小时
...
dc_charge_load_profile_raw = np.array([])
OG_flag = 0 
GC_flag = 0
# 数据变量
irrad_profile = np.array([])
dc_charge_load_profile_year = np.array([])
dc_park_load_profile = np.array([])
ac_charge_load_profile_year = np.array([])
ac_park_load_profile = np.array([])
dc_charge_load_profile = np.array([])
res = None
res_grid = None
n_hours = 0
# 最终选定方案参数
...
def preprocess_inputs(typical_day_avg_load, signals=None):
    ...
    global Delta_t
    global electricity_price, grid_co2_factor
    ...
    global irrad_profile, dc_charge_load_profile_year, dc_park_load_profile
    global ac_charge_load_profile_year, ac_park_load_profile
    global dc_charge_load_profile, res, res_grid, n_hours
    global pv_ac_selected, pv_dc_selected, eb_ac_final, eb_dc_final
    global charger_ac_selected, charger_dc_selected
...
# def run_microgrid_optimization(xl_user, xu_user,
#                                 irrad_profile,
#                                 dc_charge_load_profile_year,
#                                 dc_park_load_profile,
#                                 ac_charge_load_profile_year,
#                                 ac_park_load_profile,
#                                 dc_charge_load_profile,
#                                 charge_rate_ratio,
#                                 discharge_rate_ratio,
#                                 Delta_t,
#                                 AC_to_DC_conversion_efficiency,
#                                 DC_to_AC_conversion_efficiency,
#                                 eb_discharge_eff,
#                                 eb_charge_eff,
#                                 AC_EV_charger_unit_price,
#                                 DC_EV_charger_unit_price,
#                                 AC_Energy_storage_system_unit_price,
#                                 DC_Energy_storage_system_unit_price,
...
def show_OG_selected_solution():    
    ...
    global pv_ac_selected, pv_dc_selected, eb_ac_final, eb_dc_final
    global charger_ac_selected, charger_dc_selected
    # def show_selected_solution(res,
    #                            irrad_profile,
    #                            dc_charge_load_profile_year,
    #                            dc_park_load_profile,
    #                            ac_charge_load_profile_year,
    ...
    for t in range(n_hours):
        ...
        pv_wasted = pv_wasted_ac + pv_wasted_dc
        if pv_wasted > 0:
            pv_curtailment_hour += pv_wasted * Delta_t                 

        pv_curtailment += pv_curtailment_hour
        grid_energy += grid_energy_hour
        # 记录SOC
        soc_ac_array[t] = soc_ac
        soc_dc_array[t] = soc_dc
        grid_energy_profile[t] = grid_energy_hour
        pv_curtailment_profile[t] = pv_curtailment_hour
...
def run_GC_optimization():
    ...
    global pv_ac_selected, pv_dc_selected, eb_ac_final, eb_dc_final
    global charger_ac_selected, charger_dc_selected
    # def optimize_storage_capacity_grid(xl_user, xu_user,
    #                                     pv_ac_selected, pv_dc_selected,
    #                                     charger_ac_selected, charger_dc_selected,
    #                                     irrad_profile,dc_charge_load_profile,
    #                                     dc_charge_load_profile_year, dc_park_load_profile,
    #                                     ac_charge_load_profile_year, ac_park_load_profile,
    #                                     charge_rate_ratio, discharge_rate_ratio,
    #                                     Delta_t, electricity_price, grid_co2_factor,
    #                                     AC_to_DC_conversion_efficiency, DC_to_AC_conversion_efficiency,
    #                                     eb_discharge_eff, eb_charge_eff,
    #                                     AC_EV_charger_unit_price, DC_EV_charger_unit_price,
    #                                     AC_Energy_storage_system_unit_price, DC_Energy_storage_system_unit_price,
    #                                     annual_maintenance_rate,n_hours):
...
# def simulate_grid_mode_selection(res_grid, pv_ac_selected, pv_dc_selected, charger_ac_selected, charger_dc_selected,
#                                  irrad_profile, dc_charge_load_profile_year, dc_park_load_profile,
#                                  ac_charge_load_profile_year, ac_park_load_profile,
#                                  charge_rate_ratio, discharge_rate_ratio,
#                                  Delta_t, electricity_price,
#                                  AC_to_DC_conversion_efficiency, DC_to_AC_conversion_efficiency,
#                                  eb_discharge_eff, eb_charge_eff):
def show_GC_selected_solution():
    global xl_user, xu_user, park_space, car_number
    global Delta_t
    global electricity_price, grid_co2_factor
    global eb_charge_eff, eb_discharge_eff
    ...
    while True:
        ...
        eb_ac, eb_dc = res_grid.X[choice_grid]
        max_ac_charge_power = eb_ac * charge_rate_ratio
        max_dc_charge_power = eb_dc * charge_rate_ratio
        max_ac_discharge_power = eb_ac * discharge_rate_ratio
        max_dc_discharge_power = eb_dc * discharge_rate_ratio

        pv_power_ac = pv_ac_selected * irrad_profile
        pv_power_dc = pv_dc_selected * irrad_profile

        soc_ac_array = np.zeros(n_hours)
        soc_dc_array = np.zeros(n_hours)
        grid_energy_profile = np.zeros(n_hours)
        ...
        plt.figure(figsize=(12, 6))
        plt.plot(np.arange(24), ac_load_24, label='AC Load (kW)')
        plt.xlabel('Hour')
        plt.ylabel('Load Power (kW)')
        plt.title('AC Load Power Profile (First 24h)')
        plt.legend()
        plt.grid(True)
        plt.show()

        plt.figure(figsize=(12, 6))
        plt.plot(np.arange(24), dc_load_24, label='DC Load (kW)')
        plt.xlabel('Hour')
        plt.ylabel('Load Power (kW)')
        plt.title('DC Load Power Profile (First 24h)')
        plt.legend()
        plt.grid(True)
        plt.show()

        plt.figure(figsize=(12, 6))
        plt.plot(np.arange(24), grid_energy_24, label='Grid Energy (kW)')
        plt.xlabel('Hour')
        plt.ylabel('Grid Power (kW)')
        plt.title('Grid Energy Profile (First 24h)')
        plt.legend()
        plt.grid(True)
        plt.show()
